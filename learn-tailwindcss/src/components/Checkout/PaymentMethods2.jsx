  ...
  return (
    <div className="mt-6">
        ...
        {methods.map((method) => (
          <div
            ...
            className={`cursor-pointer border rounded-xl p-4 flex flex-col items-center justify-center gap-2 transition
              ${paymentMethodsOption === method.id ? "border-primary bg-primary/10 scale-105" : "hover:border-primary hover:scale-105"}
            `}
          >
            {method.icon}
      ...
      {paymentMethodsOption === "efectivo" && (
        <div className="mt-6 p-5 border rounded-xl bg-base-100 shadow-sm">
          {/*<h4 className="font-bold mb-4 text-center">¿Cómo recibís el pedido?</h4>
          <div className="flex flex-col gap-3">
            {['delivery', 'pickup'].map((option) => (
              <label key={option} className={`flex items-center gap-3 p-3 border rounded-lg cursor-pointer transition ${cashMethod === option ? 'border-primary bg-primary/5' : 'hover:bg-base-200'}`}>
                <input type="radio" name="deliveryMethod" className="radio radio-primary" checked={cashMethod === option} onChange={() => setCashMethod(option)} />
                <span className="font-medium">{option === 'delivery' ? 'Envío a domicilio' : 'Retiro por local'}</span>
              </label>
            ))}
          </div>
          {cashMethod && (
